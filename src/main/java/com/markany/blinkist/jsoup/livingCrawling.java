...
public class livingCrawling {
	...
	public static void main(String args[]) {
		...
		livingCrawling crawling = new livingCrawling();
	...
	public void Info() {
		...
		try {
			...
			Document document1 = Jsoup
					.connect("https://book.naver.com/bestsell/bestseller_list.nhn?type=image&cp=yes24&cate=001001001")
					.get();
			...
			for (int i = 0; i < 22; i++) {
				...
				if (i == 2 || i == 3 || i == 4 || i == 5 || i == 7 || i == 9 || i == 10 || i == 14 || i == 15 || i == 16
						|| i == 18 || i == 19)
					continue;

				System.out.println("카테고리: 건강/생활/요리");
				...
				/*
				 * AuthorVo vo = new AuthorVo(); if(authorservice.checkName(name)!=null)
				 * {//작가이름이 이미있다면 continue; } vo.setName(name); //작가이름
				 * vo.setIntroduce(introduce);//작가소개 authorservice.insertAuthor(vo);
				 * ctx.close();
				 */
				 ...
				 * bookvo.setCategory("건강/생활/요리"); bookvo.setImg_path(img_path);
				 * 
				 ...
				 * 
				 * //책의 콘텐츠저장 ContentVo contentvo = new ContentVo(); for(int k=0; k<lim; k++) {
				 * contentvo.setChapter_title(chapter_title.get(k));
				 * contentvo.setContent(content.get(k)); int maxNumber =
				 * contentService.selectMaxChapter_no(); contentvo.setChapter_no(maxNumber+1);
				 * contentvo.setBook_no(count); contentService.insertContent(contentvo);
				 * 
				 * }
				 */
			}
